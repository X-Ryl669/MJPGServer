...
namespace Stream
{
    ...
    public:
        ...
        template <typename T>
        bool save(const T & val) throw() { return write(&val, (const uint64)sizeof(val)) == (uint64)sizeof(val); }
        /** A useful helper when what you write is typed and you don't react on errors, just bailing out.
            @param val  The value to write onto
            @return true when val was completely written into the stream */
        template <typename T, size_t N>
        bool save(const T (&val)[N]) throw() { return write(&val[0], (const uint64)(sizeof(val[0]) * N)) == (uint64)(sizeof(val[0]) * N); }
        /** A useful helper for writing strings to this stream.
            @param val  The string to write to the stream.
            @return true when val was completely written into the stream */
        bool save(const Strings::FastString & val) throw() { return write((const unsigned char*)val, (const uint64)val.getLength()) == (uint64)val.getLength(); }
        /** A useful helper to write a memory block to this stream.
            @param val  The memory block to write to the stream.
            @return true when val was completely written into the stream */
        bool save(const Utils::MemoryBlock & val) throw() { return write(val.getConstBuffer(), (const uint64)val.getSize()) == (uint64)val.getSize(); }
    };
