...
namespace Exception
{
    ...
    struct WithStackM { static TLSDecl bool w; static bool shouldSkipCS() { return w; } };

    struct ScopeWithoutStack { ScopeWithoutStack() { WithStackM::w = true; } ~ScopeWithoutStack() { WithStackM::w = false; } };
    ...
    /** Capture the stack, if available on the platform.
        ...
        So the usual exception code like this:
        @code
        struct Except // optional : public std::exception
        {
             const char * msg; // This is only for this example, don't remember address of objects on stack
...
#define tryNoStack     for (::Exception::ScopeWithoutStack _ww##__LINE__;::Exception::WithStackM::w;::Exception::WithStackM::w = false) try
