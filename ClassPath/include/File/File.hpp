    ...
    */
    ...
    public:
        ...
        /** Change the working directory
            @sa ScopedDirChange
            @note To get the current working directory use General::getSpecialPath(General::Current) */
    ...
    public:
        ...
        typedef Strings::StringArray                            NameArray;
...
//        mutable struct dirent       data;
    ...
    struct General
    {
        ...
        /** Compute relative path between two paths. Both path does not need to exist, but they must be absolute path.
            @param rootPath     The path to use as the root (this method will return the relative path to apply on rootPath to reach targetPath)
            @param targetPath   The path to reach
            @return A relative path that, if applied on rootPath reach targetPath */
        static String relativePath(const String & rootPath, const String & targetPath);
    ...
    /** Change the working directory for the given scope and resume to previous directory upon leaving the scope. */
    struct ScopedDirChange
    {
        String prevDir;
        bool success;
        /** Check if directory changed */
        inline bool succeeded() const { return success; }
        ScopedDirChange(const String & newDir) : prevDir(General::getSpecialPath(General::Current)), success(File::Info(newDir, true).changeWorkingDir()) {}
        ~ScopedDirChange() { if (success) File::Info(prevDir, false).changeWorkingDir(); }
    };
