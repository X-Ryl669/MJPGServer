...
namespace Arguments
{
  ...
  #define TRANS(X) X
    ...
    /** An action that's modifying a property of some introspectable object. */
    template <typename Class>
    struct PropertyAction : public BaseAction
    {
        ...
        {
            if (valuesCount != desc.requiredValues || valuesCount != 1 || !values) return TRANS("Invalid number of arguments for option: ") + TRANS(desc.longTrigger);
            // Ask for the property type
            Signal::Introspect<Class> * introspection = dynamic_cast<Signal::Introspect<Class> *>(&obj);
            if (!introspection) return TRANS("The mapped property is not found in the introspection table (should fix the code)");
            // If the compiler stops here, it's likely because there is no known conversion from String to your property type
            bool ret = introspection->setPropertyUnsafe(propertyIndex, values[0]);
            return ret ? String("") : TRANS("Could not set the property for: " + desc.longTrigger);
        }

        PropertyAction(Class & obj, const char * propertyName) : obj(obj), propertyIndex((size_t)-1)
        {
            Signal::Introspect<Class> * introspection = dynamic_cast<Signal::Introspect<Class> *>(&obj);
            if (introspection) const_cast<size_t&>(propertyIndex) = introspection->findPropertyByName(propertyName);
        }
    };
    ...
    /** Register actions and description for it.
        This is used for mapping an argument to an object property, in effect, if the argument is found, it set the property of the given object to the argument value (required).

        @param obj                  The object instance. The object's class must derive from Signal::Introspect<>
        @param propertyName         The property name. Make sure the property exists or it'll fail at runtime
        @param helpText             The text to display for this argument when the user use --help or -h
        @param longTrigger          The long trigger text (usually what's after -- in the argument help description)
        @param numberOfArguments    The number of arguments expected for this option. This is typically 1 but for bool type, it can be 0
        @param shortTrigger         The short trigger text (usually what's after - in the argument help description). If ommited, the system allocate one for you */
    template <typename Obj>
    inline void declareProp(Obj & obj, const char * propertyName, const char * helpText, const char * longTrigger, const char * shortTrigger = 0)
    {
        // First create the descriptor
        Descriptor * desc = new Descriptor(helpText, shortTrigger ? shortTrigger : "", longTrigger, 1);
        Core::registerDescriptorAndAction(desc, new PropertyAction<Obj>(obj, propertyName));
    }
