...
namespace Threading
{
    ...
    public:
        /** Thread should run */
        inline void start() volatile            { run.save(true); }
        /** Is the thread running ? */
        inline bool isRunning() const volatile  { return run.read(); }
        /** Thread should stop */
        inline void stop() volatile             { run.save(false); }
        /** Construction */
...
#define GetLocalVariable(X, Y, key) X * Y = 0; { Threading::Thread::LocalVariableImpl<X>* tmpVar = dynamic_cast< Threading::Thread::LocalVariableImpl<X>* >(Threading::Thread::getLocalVariable(key)); Y = tmpVar ? tmpVar->operator X*() : 0; }
    ...
    public:
        ...
        /** Thread quiescient state callback */
        struct Quiescent
        {
            /** This is called when "isRunning()" is called.
                Thus no lock should be taken when this method is called.
                If you need to assert if the thread is running while you are holding some locks, you'll do
                so via checkRunningVar() method instead */
            virtual void quiescentState(const Thread * us) = 0;
            ~Quiescent() {}
        };
    ...
    protected:
        ...
        Leaving *               leaving;
        /** The thread quiescent state callback, if provided */
        Quiescent *             quiescent;
    ...
    private:
        ...
        /** The thread cleanup state. This is never accessed from the thread, thus is correctly non protected.
            This is used to mark when the thread is cleaned */
        bool                    cleanupState;
        /** This thread lock */
        mutable  Lock           _lock;
    ...
    public:
        ...
        /** Signal the thread should stop.
            This is not required in most cases. This simply change the running condition to false.
            You still need to call destroyThread on the thread that created or is managing this thread. */
        void signalShouldStop() volatile;
        /** The needed override */
        virtual uint32 runThread() = 0;
        /** Is the thread running ?
            @warning You should not call this in a scoped lock section if you have installed a quiescent state handler.
                     If you actually need to check the state, use checkRunningVar() instead */
        virtual bool isRunning() const;
        /** Fetch the running var value. */
        bool checkRunningVar() const { return run.isRunning(); }
    ...
    public:
        ...
        /** Get the current Thread pointer. The main thread does not have such a pointer */
        static Thread * getCurrentThread();

#ifdef _WIN32
        /** Static key to store "this" in thread specific data */
        static DWORD         threadThisKey;
    ...
    public:
        ...
        /** Set the thread's quiescent state callback. You usually avoid dealing with this directly.
            @param cb A pointer to a non-owned callback that'll be called when the thread is quiescent. Set to 0 to remove the callback.
            @warning The thread does not own the callback pointer that should still exist when the thread is leaving. */
        inline void setQuiescentCallback(Quiescent * cb) { ScopedLock scope(_lock); quiescent = cb; }
