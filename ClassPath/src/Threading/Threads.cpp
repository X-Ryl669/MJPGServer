...
namespace Exception { TLSDecl bool WithStackM::w; }
...
#endif
#ifdef _WIN32
    thread(NULL), threadID(0), leaving(0), run(false), cleanupState(true), _lock(name) {}
#else
    leaving(0), quiescent(0), run(false), cleanupState(true), _lock(name), isCreated(false)
{
...
{
    ScopedLock scope(_lock);
    if (run.isRunning())
    {
        // Need to release the RunCondition object lock
        ScopedUnlock unlock(_lock);
    ...
    {   run.stop();   thread = NULL; threadID = 0; cleanupState = true; return false; }
    ...
    {
        run.stop();   memset((void*)&thread, 0, sizeof(thread)); cleanupState = true; return false;
    }
...
{
    if (quiescent)
        // This is not safe if modified from outside, but since this should never happen in most cases
        // (either it's the thread itself that install its quiescent pointer, or it's done before the thread is running), it's ok
        quiescent->quiescentState(const_cast<Thread*>(this));
    return run.isRunning();
}

void Thread::signalShouldStop() volatile
{
    run.stop();
}
...
bool Thread::destroyThread(const bool & rcbDontWait) volatile
{
    ...
    // Check if the thread is not running anymore and ready to be cleaned up
    if (!cleanupState && !run.isRunning())
    {
        ScopedLock scope(_lock);
        if (leaving) leaving->threadLeaving(const_cast<Thread*>(this));
#ifdef _WIN32
        // Close the handles if needed
        TlsSetValue(threadThisKey, 0);
        if (thread != NULL)
        {
        ...
        if (isTIDValid((HTHREAD*)&thread)) memset((void*)&thread, 0, sizeof(thread));
        SemDestroy(semaphore);
        SemInit(semaphore, 0);
#endif
        // Ok, now we are done
...
{
    if (threadThisKey == TLS_OUT_OF_INDEXES)
        threadThisKey = TlsAlloc();
}

Thread * Thread::getCurrentThread()
{
    createTLSThisKey();
    return TlsGetValue(threadThisKey);
}
#elif defined(_POSIX)
...
Thread * Thread::getCurrentThread()
{
    return (Thread*)pthread_getspecific(threadThisKey);
}
#endif
